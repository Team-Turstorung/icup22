...
class SimplesSolverMultipleTrains(Solution):
    ...
    def navigate_train(self, all_shortest_paths: Dict[str, Tuple], train: MultiTrain, round_action: RoundAction):
        ...
        if len(train.path) == 1:
            ...
            if train.reserved_capacity != 0:
                passenger_groups = [self.network_state.passenger_groups[passenger_group_id] for passenger_group_id in self.network_state.stations[train.position].passenger_groups]
    ...
    def plan_train(self, all_shortest_paths: Dict[str, Tuple], train: MultiTrain):
    ...
    def schedule(self) -> Schedule:
        ...
        self.network_state: MultiNetworkState = multify_network(self.network_state)
        ...
        while True:
            ...
            for train in [train for train in self.network_state.trains.values() if train.station_state == StationStateType.ARRIVING]:
            ...
            for train in sorted([train for train in self.network_state.trains.values() if not train.station_state == StationStateType.LEAVING], key=lambda train: train.speed, reverse=True):
                if len(train.path) != 0:
                    self.navigate_train(all_shortest_paths, train, round_action)

            processed = set()
            for pair in combinations([train.name for train in self.network_state.trains.values() if train.station_state == StationStateType.BLOCKED_STATION], 2):
            ...
            leaving_trains = {train for train in self.network_state.trains.values() if train.station_state == StationStateType.LEAVING}
            while len(leaving_trains) != 0:
                leaving_train = leaving_trains.pop()
                for blocked_train in [train for train in self.network_state.trains.values() if train.station_state == StationStateType.BLOCKED_STATION]:
